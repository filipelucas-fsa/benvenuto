...
import { X, Minus, Plus, Trash2, Store, Bike, UtensilsCrossed, ShoppingBag, Tag, XCircle } from "lucide-react";
import { useCart } from "@/lib/store/cart";
import { montarMensagemWhatsApp, gerarLinkWhatsApp } from "@/lib/whatsapp";
import { validarCupomAction, registrarUsoCupomAction } from "@/lib/coupons/actions";
...
export default function CartDrawer() {
  const {
    items, isOpen, close, removeItem, updateQuantidade, subtotal, total, clear,
    cupomAplicado, descontoAplicado, aplicarCupom, removerCupom,
  } = useCart();
  ...
  const [mensagemCupom, setMensagemCupom] = useState<{ texto: string; sucesso: boolean } | null>(null);
  const [validandoCupom, iniciarValidacaoCupom] = useTransition();
  ...
  const aplicarCodigoCupom = () => {
    if (!codigoCupom.trim()) return;
    setMensagemCupom(null);
    iniciarValidacaoCupom(async () => {
      const resultado = await validarCupomAction(
        codigoCupom,
        items.map((i) => ({
          produtoId: i.produto.id,
          categoriaId: i.produto.categoria_id,
          precoUnitario: i.produto.preco_promocional ?? i.produto.preco,
          quantidade: i.quantidade,
        }))
      );

      if (!resultado.valido || !resultado.cupom || resultado.descontoAplicado == null) {
        setMensagemCupom({ texto: resultado.mensagem, sucesso: false });
        return;
      }

      aplicarCupom(resultado.cupom, resultado.descontoAplicado);
      setMensagemCupom({ texto: resultado.mensagem, sucesso: true });
    });
  };
  ...
  const finalizarPedido = () => {
    ...
    if (cupomAplicado && descontoAplicado > 0) {
      // Não bloqueia o envio do pedido se isso falhar — é só telemetria de uso.
      registrarUsoCupomAction({
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          {items.length > 0 && (
            <div className="mt-6 space-y-3">
              ...
              <div>
                {cupomAplicado ? (
                  <div className="flex items-center justify-between rounded-lg border border-benvenuto-green/30 bg-benvenuto-green/10 px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <Tag size={15} className="text-benvenuto-green" />
                      <div>
                        <p className="text-sm font-semibold text-benvenuto-green">{cupomAplicado.codigo}</p>
                        <p className="text-xs text-benvenuto-cream/50">
                          -{formatarPreco(descontoAplicado)} de desconto
                        </p>
                      </div>
                    </div>
                    <button
                      ...
                      className="focus-ring text-benvenuto-cream/40 hover:text-benvenuto-red"
                    >
                      <XCircle size={17} />
                    </button>
                  </div>
                  ...
                  <div>
                    <div className="flex gap-2">
                      <input
                        ...
                        onKeyDown={(e) => e.key === "Enter" && aplicarCodigoCupom()}
                        placeholder="Cupom de desconto"
                        className="focus-ring flex-1 rounded-lg border border-benvenuto-gold/15 bg-benvenuto-black px-3 py-2.5 text-sm uppercase tracking-wide text-benvenuto-cream placeholder:text-benvenuto-cream/30 placeholder:normal-case"
                      />
                      <button
                        onClick={aplicarCodigoCupom}
                        disabled={validandoCupom || !codigoCupom.trim()}
                        className="focus-ring shrink-0 rounded-lg border border-benvenuto-gold/25 px-4 text-sm font-semibold text-benvenuto-cream transition-colors hover:border-benvenuto-gold disabled:opacity-40"
                      >
                        {validandoCupom ? "..." : "Aplicar"}
                      </button>
                    </div>
                    {mensagemCupom && (
                      <p className={`mt-1.5 text-xs ${mensagemCupom.sucesso ? "text-benvenuto-green" : "text-benvenuto-red"}`}>
                        {mensagemCupom.texto}
                      </p>
                    )}
                  </div>
        ...
        {items.length > 0 && (
          <div className="border-t border-benvenuto-gold/10 px-5 py-4">
            <div className="mb-3 space-y-1.5 text-sm">
              <div className="flex items-center justify-between text-benvenuto-cream/60">
                <span>Subtotal ({totalItens} {totalItens === 1 ? "item" : "itens"})</span>
                <span>{formatarPreco(subtotal())}</span>
              </div>
              {cupomAplicado && descontoAplicado > 0 && (
                <div className="flex items-center justify-between text-benvenuto-green">
                  <span>Desconto ({cupomAplicado.codigo})</span>
                  <span>-{formatarPreco(descontoAplicado)}</span>
                </div>
              )}
              <div className="flex items-center justify-between border-t border-benvenuto-gold/10 pt-1.5">
                <span className="font-semibold text-benvenuto-cream">Total</span>
                <span className="font-display text-lg font-bold text-benvenuto-gold">{formatarPreco(total())}</span>
              </div>
            </div>
            <button
